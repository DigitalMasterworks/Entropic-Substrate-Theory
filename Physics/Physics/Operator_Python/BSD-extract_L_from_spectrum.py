...
to v2 pipeline (jitter, deflation, robust scalar calibration).
...
def fit_freq_response(rows: List[CoeffRow], deg: int, Pcal: int, use_p2: bool, lam: float, trim_k: float):
 ...
 y.append(1.0/max(r.amp_raw, 1e-15))
...
def apply_freq_response(rows: List[CoeffRow], coeffs):
 ...
 s_omega = float(Phi @ coeffs)
 s_omega = max(s_omega, 1e-6)
 r.amp_cal = r.amp_raw * s_omega
...
def multiplicativity_samples(
 ...
 amp_base = amp_raw * s_of_omega(fr_coeffs, omega) * scalar_s
 omegas.append(omega)
 ...
 c = np.linalg.solve(XT_X + lamI, XT_y)
 ...
 ratio = (yhat + eps) / (yte + eps)
 logerr = np.log(yhat + eps) - np.log(yte + eps)
...
def main():
 ...
 ap.add_argument("--calib", type=str, default="median:101")
