#!/usr/bin/env python3
"""
fft_multi_null_test.py — compare prime FFT detection under multiple null modes
"""
...
def fft_prime_check(lams, Pmax=101, grid_pts=8192, snr_sigma=6.0, label=""):
 ...
 pw = power[j]
 prime_hits.append((p, pw>=thresh))

 keep = [p for (p,ok) in prime_hits if ok]
 miss = [p for (p,ok) in prime_hits if not ok]

 print(f"[{label}] threshold={thresh:.2e}")
 print(f"[{label}] primes detected: {keep}")
 print(f"[{label}] primes missed: {miss}\n")
...
def main():
 ...
 fft_prime_check(orig, label="true-spectrum")


 lams_shuf = orig.copy()
 np.random.shuffle(lams_shuf)
 fft_prime_check(lams_shuf, label="shuffled")


 fake = np.linspace(orig[0], orig[-1], orig.size)
 fft_prime_check(fake, label="constant-S")


 noisy = orig + np.random.normal(scale=np.std(orig)*0.05, size=orig.size)
 fft_prime_check(noisy, label="noisy-perturb")
