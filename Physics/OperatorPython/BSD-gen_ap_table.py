...
def ap_for_prime(p, A, B):
    # skip p=2 in this simple routine; treat it as bad
    if p == 2:
        return None, False
    # bad if p | Δ
    Δ = discriminant(A,B)
    if Δ % p == 0:
        return None, False
    # Count via sum of Legendre symbols: a_p = -∑_x χ(x^3 + A x + B)
    s = 0
    for x in range(p):
        rhs = (x*x % p * x + A*x + B) % p  # x^3 + A x + B mod p
...
def main():
    ...
    for p in primes:
        ...
        if good:
            ...
            rows.append((p, 0, 0))  # mark bad; ap not used
