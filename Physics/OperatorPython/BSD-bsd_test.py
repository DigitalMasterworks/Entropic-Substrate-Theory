...
# semicircle CDF on [-1,1]: F(t) = 1/2 + (t*sqrt(1-t^2) + arcsin t)/π
...
def sato_tate_ks(ap_dict, Pmax):
    ...
    for p, ap in ap_dict.items():
        if p <= Pmax:
            tp = (ap/np.sqrt(p))/2.0  # normalize to [-1,1]
            data.append(tp)
    if not data:
...
def rank_near_one(ap_dict, Pmax, h=0.02, k=5):
    # fit log L(1+h_j) = r*log h_j + c  (slope ~ rank)
        ...
        if args.ec_invariants:
            ...
            if N > 0:
                ...
                print(f"[FE/parity] N={N}, w={w}, estimated rank≈{r_int} → parity={parity}  "
                      f"{'OK' if ok else '⚠️ parity mismatch' if parity is not None else 'n/a'}")
