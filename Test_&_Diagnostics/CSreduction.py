...
def safe_hypot2(x, y):
    ...
    r = math.hypot(x, y)           # safe sqrt(x^2+y^2)
    r = min(r, SAFE_MAX**0.5)      # clamp huge
    r2 = r*r
...
def christoffels_closed(Sval, Sx, Sy, cval):
    ...
    # t-row (mu=0)
    G[0][0][1] =  Sx*invS  # Γ^t_{t x}
    G[0][1][0] =  Sx*invS  # symmetry in lower indices
    G[0][0][2] =  Sy*invS  # Γ^t_{t y}
    G[0][2][0] =  Sy*invS

    # x-row (mu=1)
    G[1][0][0] =  S3c2 * Sx               # Γ^x_{t t}
    G[1][1][1] = -Sx*invS                 # Γ^x_{x x}
    G[1][2][2] =  Sx*invS                 # Γ^x_{y y}
    G[1][1][2] = -Sy*invS                 # Γ^x_{x y}
    G[1][2][1] = -Sy*invS                 # Γ^x_{y x}

    # y-row (mu=2)
    G[2][0][0] =  S3c2 * Sy               # Γ^y_{t t}
    G[2][2][2] = -Sy*invS                 # Γ^y_{y y}
    G[2][1][1] =  Sy*invS                 # Γ^y_{x x}
    G[2][1][2] = -Sx*invS                 # Γ^y_{x y}
    G[2][2][1] = -Sx*invS                 # Γ^y_{y x}
...
def derive_symbolics():
    ...
    Sx = sp.Function('Sx')(X, Y)  # placeholder for clarity in printout
...
def shoot_ray(b, Xmax=400.0, h=0.1, par=None):
    ...
    x0, y0 = -Xmax, float(b)
    vx0, vy0 = 1.0, 0.0  # null gauge factor cancels; direction suffices
    ...
    if args.demo:
        ...
        for b in bs:
            a, _ = shoot_null_ray(
                ...
                pars=pars,
                S_provider=S_grad_pointmass,   # <<< was S_pointmass
        ...
        if args.plot:
            try:
                ...
                plt.legend(); plt.tight_layout(); plt.savefig("null_rays.png", dpi=150)  # pick any filename you like
                plt.close()
