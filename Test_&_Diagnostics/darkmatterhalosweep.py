...
def simulate_halo(core_radius, halo_radius, halo_strength):
    ...
    for i in range(len(dv)):
        if np.all(dv[i:] < threshold):
            flatten_index = i
            break
    flatten_radius = radii_arr[flatten_index] if flatten_index is not None else radii_arr[-1]
    v_flat = np.mean(velocities[flatten_index:])
