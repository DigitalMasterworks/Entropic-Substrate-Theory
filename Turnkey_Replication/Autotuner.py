...
if __name__ == "__main__":
    ...
    print("\n===== suggested bake in values (for reference) =====")

    # 1. Collect all parameters and rename keys to match Simulator3.py's convention (e.g., HALO_S0)
    final_params = {}

    # Halo Parameters
    halo_map = {
        "S0": "HALO_S0", "core_r": "HALO_CORE_R", "halo_inner": "HALO_INNER", 
        "halo_outer": "HALO_OUTER", "S_halo": "HALO_S_HALO", "S_core": "HALO_S_CORE",
        "G_matter": "G_MATTER", "G_lens": "G_LENS"
    }
    for k_orig, k_new in halo_map.items():
        final_params[k_new] = halo_best["params"][k_orig]

    # Void Parameters
    void_map = {
        "S_bg": "VOID_S_BG", "S_void": "VOID_S_VOID", "R": "VOID_R", 
        "wall_sigma": "VOID_WALL_SIG", "steps": "VOID_STEPS", 
        "drag": "VOID_DRAG", "launch_bias": "VOID_LAUNCH_B"
    }
    for k_orig, k_new in void_map.items():
        final_params[k_new] = void_best["params"][k_orig]

    # Anisotropy Parameters
    aniso_map = {
        "hill_dx": "ANISO_HILL_DX", "steps": "ANISO_STEPS", "G": "ANISO_G", 
        "cap": "ANISO_CAP", "Rmin": "ANISO_RMIN", "bins": "ANISO_BINS"
    }
    for k_orig, k_new in aniso_map.items():
        final_params[k_new] = aniso_best["params"][k_orig]

    # Add other fixed constants used in Simulator3.py (if not explicitly in params)
    # Simulator3.py uses VEL_CAP=1.0. ANISO_CAP is 1.0, so use a consistent name.
    final_params["VEL_CAP"] = final_params["ANISO_CAP"] # 1.0

    # Clean up and adjust ANISO_BINS / ANISO_SHELLS (Simulator3 uses fixed ANISO_BINS=24)
    # The tune block uses 'bins' and 'shell_bins'. Since Simulator3 is fixed at 24 bins,
    # and the tuning used the optimal 'shell_bins', we keep 'bins'=24 but need to store 'shell_bins'.
    # Since Simulator3 only cares about the main constants, we rely on its internal fixed values
    # for BINS and SHELLS unless they were autotuned into the main constants block.
    # We ensure ANISO_BINS is 24 as hardcoded in the original Simulator3.
    final_params["ANISO_BINS"] = 24

    # 2. Save all parameters to tuned_params.json
    try:
        with open('tuned_params.json', 'w') as f:
            json.dump(final_params, f, indent=4)
        print("\n**SUCCESS: All tuned parameters saved to tuned_params.json**")
    except Exception as e:
        print(f"\nERROR: Failed to save parameters to JSON: {e}")

    # 3. Print suggested values for console reference (as in original file)
    print(f"\n# halo")
